jQuery(document).ready(function($){
    ...
    /**
     * Credit card flag select on change
     */
    $(document).on('change', '#azpay_cc_cc_type', function () {
        mountSelect();
        numberMask();
        codeMask();
    });
     ...
     */
    function numberMask() {
      $('#azpay_cc_cc_number').mask($('#azpay_cc_cc_type').find(':selected').data('mask'));
    }

    /**
     * Mask for credit card code
     */
    function codeMask() {
      flag = $('#azpay_cc_cc_type').find(':selected').val();
      if (flag === 'amex') {
        $('#azpay_cc_cc_cid').mask('9999');
      } else {
        $('#azpay_cc_cc_cid').mask('999');
      }
    }

    /**
     * Select of parcels by credit card flag
     */
    function mountSelect() {
        parcels = $('#azpay_cc_cc_type').find(':selected').data('parcels');
        $select_parcels = $('#azpay_cc_installments');
